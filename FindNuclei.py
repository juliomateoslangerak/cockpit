...
class FindNuclei:
    def __init__(self, *args, **kwargs):
        ...
        self.site_size = mosaicWindow.window.crosshairBoxSize
    ...
    def onImage(self, data, *args):
        ...
        ret, bin = cv2.threshold(
            cv2.GaussianBlur(img, (15, 15), 3), 60, 255, cv2.THRESH_BINARY
        )
        ...
        if circles is not None:
            ...
            for cell in circles[0]:
                ...
                cellpos = (
                    position[:2]
                    + ([img.shape[0] / 2, img.shape[1] / 2] - cell[:2])
                    * self.pixel_size
                )
                ...
                stageMover.saveSite(
                    stageMover.Site(
                        sitepos, None, self.site_color, size=self.site_size
                    )
                )
