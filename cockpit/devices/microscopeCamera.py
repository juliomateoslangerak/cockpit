...
class MicroscopeCamera(MicroscopeBase, CameraDevice):
    ...
    def getImageSize(self, name):
        ...
        if not isinstance(binning, Binning):
            cockpit.util.logger.log.warning(
                "%s returned tuple not Binning()" % self.name
            )
    ...
    def makeUI(self, parent):
        ...
        events.subscribe(
            events.SETTINGS_CHANGED % self, lambda: self.updateModeButton(modeButton)
        )
        modeButton.Bind(
            wx.EVT_CHOICE, lambda evt: self.setReadoutMode(evt.GetSelection())
        )
        sizer.AddSpacer(4)
        # Gain control
    ...
    def onGainButton(self, evt):
        ...
        gain = wx.GetNumberFromUser(
            "Gain",
            "",
            "Set gain",
            value=self.settings.get("gain", 0),
            min=mingain,
            max=maxgain,
        )
        if gain == -1:
