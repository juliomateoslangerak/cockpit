        ...
        if "controller" not in self.config:
            ...
            if c_name is not None:
                try:
                    ...
                    raise Exception(
                        "%s: device not found on controller '%s'." % (self.name, c.name)
                    )
    ...
    def getHandlers(self):
        ...
        self.handlers.append(
            cockpit.handlers.lightPower.LightPowerHandler(
                ...
                {
                    "setPower": cockpit.util.threads.callInNewThread(
                        self._proxy.set_power_mw
                    ),
    ...
    def initialize(self) -> None:
        ...
        for one_letter_name in "xyz":
            ...
            if units_config_name not in self.config:
                raise Exception(
                    "missing '%s' value in the configuration" % units_config_name
                )
            units_per_micron = float(self.config[units_config_name])
            if units_per_micron <= 0.0:
                raise ValueError(
                    "'%s' configuration must be a positive value" % units_config_name
                )
        ...
        for their_axis_name in their_axes_map.keys():
            if their_axis_name not in handled_axis_names:
                ...
                # them?
                raise Exception(
                    "No configuration for the axis named '%s'" % their_axis_name
                )
