...
#
        ...
        try:
            ...
            try:
                error = json.loads(self.connection.recv(int(errorLength)))
            except ValueError:
    ...
    def ReadPosition(self, analogueChannel):
        """Returns the current position of an analogue line

        analogueChannel is an integer corresponding to the analogue in the FPGA as specified in the config files
        """
        value = self.status.getStatus(f'Analogue {analogueChannel}')
        return float(value)
