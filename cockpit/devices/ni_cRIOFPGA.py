...
FPGA_HEARTBEAT_RATE = 0.1  # At which rate is the FPGA sending update status signals
...
class NIcRIO(executorDevices.ExecutorDevice):
    ...
    def __init__(self, name, config):
        ...
        self.tickrate = (
            100  # Number of ticks per ms. As of the resolution of the action table.
        )
        # TODO: check
    ...
    def initialize(self):
        ...
        self.connection = Connection(
            parent=self, ipAddress=self.ipAddress, port=self.port, localIp=MASTER_IP
        )
    ...
    def _adaptActions(self, actions):
        ...
        maxticks = max(
            chain(
                [d[0] for d in digitals], [a[0] for a in chain.from_iterable(analogs)]
            )
        )
...
class Connection:
    ...
    def writeIndexes(
        ...
        analoguesInterleaved = [
            x for t in zip(analoguesStartIndexes, analoguesStopIndexes) for x in t
        ]
