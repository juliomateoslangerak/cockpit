...
class _LastParameters:
    """A class to keep a record of last RIM parameters using async calls."""
    ...
    @property
    def params(self):
        """Return the last recorded RIM parameters."""
    ...
    def update(self):
        ...
        if self.slm.asproxy:
            self._result = self.slm.asproxy.get_rim_sequence()


class RIM_SLM(device.Device):
    """A microscope SLM device implementing RIM logic and GUI.

    Remotely, the SLM must be running a rim_slm device server.
    ...
    .. code:: ini
        ...
        [slm]
        type: cockpit.devices.rim_slm.RIM_SLM
        uri: PYRO:rim_slm@slmhost:8000
    ...
    def finalizeInitialization(self):
        ...
        menuTuples = (
            ("Generate RIM sequence", self.testRIMSequence),
            ("RIM diff. angle", self.setDiffractionAngle),
        )
    ...
    def setEnabled(self, state):
        ...
        if state:
            # Enable.
            if self.last.params == self.getRIMsequence():
