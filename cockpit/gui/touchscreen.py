...
import numpy
import wx
from wx.lib.agw.shapedbutton import (
    SButton,
    SBitmapButton,
    SBitmapToggleButton,
    SToggleButton,
...
class SetVariable(wx.Window):
    def __init__(self, parent):
        ...
        self._text = wx.StaticText(
            self, -1, label="00.000 uuu", style=wx.ST_NO_AUTORESIZE | wx.ALIGN_CENTER
        )
...
class TouchScreenWindow(wx.Frame, mosaic.MosaicCommon):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.buttonPanel = wx.Panel(
            self.panel, -1, size=(300, -1), style=wx.BORDER_RAISED
        )
        ...
        button = self.makeToggleButton(
            ...
            "load.png",
            "unload.png",
            "Load or unload the sample from approximate in focus " + "Z position",
            (75, 75),
        )
        objectiveSizer.Add(button, 0, wx.EXPAND | wx.ALL, border=5)

        textSizer = wx.BoxSizer(wx.VERTICAL)
        sampleText = wx.StaticText(
            self.buttonPanel, -1, "Sample", style=wx.ALIGN_CENTER
        )
        sampleText.SetFont(sampleText.Font.Bold())
        textSizer.Add(sampleText, 0, wx.EXPAND | wx.ALL, border=5)

        self.sampleStateText = wx.StaticText(
            self.buttonPanel, -1, style=wx.ALIGN_CENTER
        )
        ...
        objectiveText = wx.StaticText(
            self.buttonPanel, -1, "Objective", style=wx.ALIGN_CENTER
        )
        ...
        self.objectiveSelectedText = wx.StaticText(
            self.buttonPanel, -1, style=wx.ALIGN_CENTER
        )
        ...
        for light in lightToggles:
            ...
            if powerHandler is not None:
                ...
                powerctrl.Bind(
                    ...
                    lambda evt, h=powerHandler: h.setPower(evt.Value),
                )
            ...
            expctrl.Bind(
                wx.EVT_SPINCTRLDOUBLE, lambda evt, h=light: h.setExposureTime(evt.Value)
            )
        ...
        for camera in depot.getHandlersOfType(depot.CAMERA):
            ...
            label = cockpit.gui.device.Label(parent=self.buttonPanel, label=name)
            self.camButton[i] = cockpit.gui.device.EnableButton(
                self.buttonPanel, camera
            )
        ...
        rightSideSizer.Add(wx.StaticLine(self.buttonPanel), 0, wx.ALL | wx.EXPAND, 5)
        ...
        self.canvas = cockpit.gui.mosaic.canvas.MosaicCanvas(
            self.panel, limits, self.drawOverlay, self.onMouse
        )
        ...
        for args in [
            ("Up", self.zMoveUp, None, "up.png", "Move up one Z step", (30, 30)),
            ("Inc Step", self.zIncStep, None, "plus.png", "Increase Z step", (30, 30)),
        ]:
            button = self.makeButton(self.panel, *args)
            zButtonSizer.Add(button, 1, wx.EXPAND | wx.ALL, border=2)
        ##Text of position and step size
        zPositionText = wx.StaticText(self.panel, -1, style=wx.ALIGN_CENTER)
        ...
        zStepText = wx.StaticText(self.panel, -1, style=wx.ALIGN_CENTER)
        ...
        for args in [
            ...
            ("DecStep", self.zDecStep, None, "minus.png", "Decrease Z step", (30, 30)),
        ]:
            button = self.makeButton(self.panel, *args)
    ...
    def checkBitmap(self, bitmap):
        if os.path.isfile(os.path.join(self.bitmapsPath, bitmap)):
            bmp = wx.Bitmap(os.path.join(self.bitmapsPath, bitmap), wx.BITMAP_TYPE_ANY)
        else:
            bmp = wx.Bitmap(
                os.path.join(self.bitmapsPath, "broken-file.png"), wx.BITMAP_TYPE_ANY
            )
        return bmp

    ## Create a button with the appropriate properties.
    def makeButton(
        self, parent, label, leftAction, rightAction, bitmap, helpText, size=(75, 75)
    ):
    ...
    def onAxisRefresh(self, axis, *args):
        ...
        if axis is 2:
            ...
            label = "Z Step %5.2f" % (
                cockpit.interfaces.stageMover.getCurStepSizes()[2]
            )
    ...
    def onMouse(self, event):
        ...
        if event.RightDown():
            ...
            for label, color in mosaic.SITE_COLORS:
                ...
                self.panel.Bind(
                    wx.EVT_MENU,
                    lambda event, color=color: mosaic.window.saveSite(color),
                    id=menuId,
                )
                menuId += 1
            ...
            self.panel.Bind(
                wx.EVT_MENU, lambda event: mosaic.window.setTileOverlap(), id=menuId
            )
            menuId += 1
            menu.Append(menuId, "Toggle mosaic scale bar")
            self.panel.Bind(wx.EVT_MENU, lambda event: self.togglescalebar(), id=menuId)
    ...
    def changeObjective(self):
        ...
        if h.numObjectives == 2:
            ...
            for i, objective in enumerate(h.sortedObjectives):
                ...
                menu.Bind(
                    wx.EVT_MENU, lambda evt, obj=objective: h.changeObjective(obj), id=i
                )
            cockpit.gui.guiUtils.placeMenuAtMouse(self.panel, menu)
