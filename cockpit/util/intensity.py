    ...
    def calculateInstensity(self):
        ...
        with self.openData():
            ...
            beadAverage = np.average(
                np.average(
                    self._data[:, peaky - 2 : peaky + 2, peakx - 2 : peakx + 2], axis=2
                ),
                axis=1,
            )
            ...
            self.results = dict(peak=peak, avg=avgPeak, mag=mag, phi=phi, sep=sepArr)
...
class IntensityProfilerFrame(wx.Frame):
    ...
    def __init__(self, parent=None):
        ...
        phasesTool.Bind(
            wx.EVT_SPINCTRL, lambda event: self.profiler.setPhases(event.GetInt())
        )
        phasesTool.Bind(
            wx.EVT_TEXT_ENTER, lambda event: self.profiler.setPhases(event.GetInt())
        )
        ...
        boxTool.Bind(wx.EVT_TEXT_ENTER, lambda event: self.setBoxSize(event.GetInt()))
        boxTool.Bind(wx.EVT_SPINCTRL, lambda event: self.setBoxSize(event.GetInt()))
    ...
    def loadFile(self, event):
        ...
        dlg = wx.FileDialog(top, "Open", "", "", "", wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
    ...
    def onKeys(self, event):
        keys = {
            "move": [wx.WXK_LEFT, wx.WXK_RIGHT, wx.WXK_UP, wx.WXK_DOWN,],
